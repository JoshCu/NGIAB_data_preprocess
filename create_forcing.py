...
def merge_zarr_stores(urls, start_time, end_time):
    ...
    delayed_stores = [
        dask.delayed(
            lambda url: xarray.open_zarr(s3fs.S3Map(url, s3=s3fs.S3FileSystem(anon=True))).sel(
                time=time_slice
            )
        )(url)
        for url in urls
...
def add_spatial_metadata(ds):
    ...
    ds.rio.write_crs(ds_meta.crs.attrs["spatial_ref"], inplace=True).rio.set_spatial_dims(
        x_dim="x",
        y_dim="y",
        inplace=True,
...
def clip_dataset(ds, gdf):
    ...
    out_grid = make_geocube(
        vector_data=gdf, measurements=["cat"], like=ds  # ensure the data are on the same grid
    )
...
def compute_zonal_stats(ds, gdf, catchment_ids):
    with LocalCluster().get_client() as client:
        ...
        delayed = [
            dask.delayed(perform_zonal_computation)(scattered_ds, cat_id)
    ...
    for dat in results:
        for cat in dat:
            ...
            with open(f"{curent_dir}/output/{wb_id}/forcings/{cat}.csv", "w") as f:
                ...
                df.to_csv(f, columns=column_names)
