...
async function get_boundary(vpu_code){
    ...
    // above bounds do not match the tile bounds
    // calculate the bounds from the tile bounds
    const proxyUrl = 'http://localhost:5000/get_map_data';
    url = baseUrl + vpu_code + '@png/18/0/0.png';
    // fetch will 404 and return the bounds in the body 
    //Coverage [minx,miny,maxx,maxy] is [53410, 154705, 65609, 162724, 18], index [x,y,z] is [0, 0, 18]

    try {
        const response = await fetch(`${proxyUrl}?url=${encodeURIComponent(url)}`);
        bounds = await response.json();
        console.log('bounds ' + bounds + ' for ' + vpu_code);
    } catch (error) {
